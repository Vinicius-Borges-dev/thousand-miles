...
class UserController:
    ...
    def create_user(self, req, res):
        self.__name = req.form["name"]
        self.__email = req.form["email"]
        self.__password = req.form["password"]
        self.__cpf = req.form["cpf"]
        self.__rg = req.form["rg"]
        self.__birth_date = req.form["birth_date"]
        ...
        if (
            ...
            return res({"status": "error", "message": "Campos não foram preenchidos!"}),422
    ...
    def login(self, req, res):
        ...
        if not user: 
            return (
                res({
                    "status": "error",
                    "message": "Usupario não encotrado!"
                }), 404
            )
            
        user_dict = self.__user_model.to_dict(user)
        
        if not self.__user_middleware.verify_passwords_match(user_dict.password, self.__password):
            return (
                res({
                    "status": "erro",
