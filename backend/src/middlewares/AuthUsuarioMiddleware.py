...
class AuthUsuarioMiddleware:
    ...
    def verificar_email(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if usuario:
                return jsonify({"status": "erro", "mensagem": "Tente com outras credenciais"}), 400
    ...
    def validar_existencia_token(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not token:
                return jsonify({"status": "erro", "mensagem": "Autorização não está presente."}), 400
            return f(*args, **kwargs)
        return wrapper
    ...
    def validar_dados_token(f):
        @wraps(f)
        def wrapper(*args, **kwargs):
            if not AuthUsuarioMiddleware._extrair_e_verificar_token():
    ...
    def capturar_permissao_de_funcionario(f):
        @wraps(f)
        def wrapper(*args, **kwargs):
            dados_token = AuthUsuarioMiddleware._extrair_e_verificar_token()
            if not dados_token:
                return jsonify({"status": "erro", "mensagem": "Acesso negado."}), 401
            if dados_token.get("nivel_acesso") != "admin":
                return jsonify({"status": "erro", "mensagem": "Você não tem permissão para realizar essa operação."}), 401
            return f(*args, **kwargs)
        return wrapper
